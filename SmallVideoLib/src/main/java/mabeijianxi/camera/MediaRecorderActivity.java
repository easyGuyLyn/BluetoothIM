...
public class MediaRecorderActivity extends Activity implements
     ...
     * 录制完跳回去
     */
    public final static String MEDIA_RECORDER_RETURN_KEY = "media_recorder_return_key";

    /**
     * @param context
     * @param overGOActivityName 录制结束后需要跳转的Activity全类名
     */
    public static void goSmallVideoRecorder(Activity context, String overGOActivityName, MediaRecorderConfig mediaRecorderConfig, Boolean isJustReturn, int resultCode) {
        ...
        if (isJustReturn) {
            context.startActivityForResult(new Intent(context, MediaRecorderActivity.class)
                    ...
                    .putExtra(MEDIA_RECORDER_RETURN_KEY, isJustReturn), resultCode);
        } else {
            context.startActivity(new Intent(context, MediaRecorderActivity.class)
                    .putExtra(OVER_ACTIVITY_NAME, overGOActivityName)
                    .putExtra(MEDIA_RECORDER_CONFIG_KEY, mediaRecorderConfig)
                    .putExtra(MEDIA_RECORDER_RETURN_KEY, isJustReturn));
        }
    }
...
//		TODO 开始录制的图标
    ...
    public void onEncodeComplete() {
        ...
        try {
            ...
            //如果上个页面等于跳转的页面
            if (getIntent().getBooleanExtra(MEDIA_RECORDER_RETURN_KEY, false)) {
                intent = new Intent();
